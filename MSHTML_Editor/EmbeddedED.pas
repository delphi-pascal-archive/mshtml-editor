...
uses
  Windows, Classes, ActiveX, Forms, MSHTML_TLB,
...
type
  ...
  TSnapRect = procedure(Sender: TObject; const pIElement: IHTMLElement; var prcNew: tagRECT; eHandle: _ELEMENT_CORNER; var Result: HResult) of object;
  ...
  protected
    ...
    procedure DocumentComplete(Sender: TObject; const pDisp: IDispatch; var URL: OleVariant; var HandlingCompleat: Boolean); virtual;
  ...
  published
    ...
    property OnSnapRect: TSnapRect read FExtSnapRect write FExtSnapRect;
...
begin
  ...
  TEditHost(FEditHost).FExtSnapRect := FExtSnapRect;
...
begin
  ...
  { all messages to MSHTML comes trough heir - KEEP IT LEAN.
...
//------------------------------------------------------------------------------
procedure TEmbeddedED.DocumentComplete(Sender: TObject; const pDisp: IDispatch; var URL: OleVariant; var HandlingCompleat: Boolean);
var
  IW: IwebBrowser2;
  adoc: IhtmlDocument2;
  iDocDisp: IDispatch;
  ...
  begin
     ...
     FMarkUpServices := Doc as MSHTML_TLB.IMarkupServices;
...
begin
  ...
  { a derived component need a way to know if this DocumentComplete is completely
    ...
    If this eventhandler isn't exited then HandlingCompleat is set to false at
    the very end }
  HandlingCompleat := true;
  ...
  if FEditMode
     then begin
        ...
        if F2DPosition
           then CmdSet_B(IDM_2D_POSITION, true);

        if FLiveResize
           then CmdSet_B(IDM_LIVERESIZE, true);
...
begin
  ...
  //IpropertyNotifySink is automatic connected, so we do nothing heir
end;
//------------------------------------------------------------------------------
...
begin
  { Called by MSHTML to obtain the host's IDispatch interface.
    There is a sample on how to use it heir:
...
begin
  ...
  {$ELSE}
    //Navigate(Url); //this is a bit unstable heir so we use a URLmoniker instead
...
begin
  ...
  if TwebBrowser(Self).Document <> nil
     then begin
        //we need to catch a few special commands heir
        case cmdID of
...
begin
  ...
  //we need to catch some commands heir
  case cmdID of
...
//               aElement.ClassName := 'KS_DeleteMe'; // RHR
              aElement._className := 'KS_DeleteMe'; // RHR
     ...
     while true do
        begin
           ...
           if aElement._ClassName = 'KS_DeleteMe'
              then begin
                 aDomNode := aElement as IHTMLDomNode;
                 aDomNode.removeNode(false); //false = do not remove child nodes
              end
  ...
  begin
     ...
     while assigned(aElement) do
        begin
           if Length(aElement._className) > 0   // RHR
              then begin
                 if FStyles.Find(aElement.tagName + '.' + aElement._className, I)  // RHR
              ...
              try
                 while assigned(aElement) do
                    begin
                       ...
                       aElement._className := aStyle;    // RHR
...
begin
  //asm int 3 end; //trap
  Result := AnsiSameText(FActualElement._ClassName, ClassName)   // RHR
end;
//------------------------------------------------------------------------------
...
begin
  ...
  FDisplayPointerStart.MoveToMarkupPointer(FMarkupPointerStart as MSHTML_TLB.IMarkupPointer, nil);
  FDisplayPointerEnd.MoveToMarkupPointer(FMarkupPointerEnd as MSHTML_TLB.IMarkupPointer, nil);
...
var
  sFindThis: string;
  FP: MSHTML_TLB.IMarkupPointer; //points to end of searched string - if found
...
begin
  ...
  { we handle Accelerators in IDOCHOSTUIHANDLER:TranslateAccelerator witch
    ...
    If MSHTML handle the key we don't come heir !  }
